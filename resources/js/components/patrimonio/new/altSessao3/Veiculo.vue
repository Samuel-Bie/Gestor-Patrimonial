                ...
                <div>
                    <v-container class="pa-0" grid-list-xl fluid>
                        <v-layout row wrap m0>
                            <v-flex xs12 m0>
                                <span class="heading-4 text-info">Informacao adicional do bem veículo</span>
                            </v-flex>
                        </v-layout>
                    </v-container>
                            ...
                            <v-layout align-end wrap>
                                ...
                                <v-flex xs12 md3>
                                    <v-select
                                        v-model="ferramentas"
                                        :items="['SIM', 'NAO']"
                                        :error-messages="ferramentasErrors"
                                        label="KIT Ferramentas"
                                        required
                                        @change="$v.ferramentas.$touch()"
                                        @blur="$v.ferramentas.$touch()">
                                    </v-select>
                                </v-flex>
                                ...
                                <v-flex xs12 md3>
                                    <v-select
                                        v-model="extintor"
                                        :items="['SIM', 'NAO']"
                                        :error-messages="extintorErrors"
                                        label="Extintor de Incendios"
                                        required
                                        @change="$v.extintor.$touch()"
                                        @blur="$v.extintor.$touch()">
                                    </v-select>
                                </v-flex>
                                ...
                                <v-flex xs12 md3>
                                    <v-select
                                        v-model="assistencia"
                                        :items="['SIM', 'NAO']"
                                        :error-messages="assistenciaErrors"
                                        label="Assistencia Tecnica"
                                        required
                                        @change="$v.assistencia.$touch()"
                                        @blur="$v.assistencia.$touch()">
                                    </v-select>
                                </v-flex>
                                ...
                                <v-flex xs12 md3>
                                    <v-text-field
                                        v-model="garantia"
                                        label="Garantia"
                                        :error-messages="garantiaErrors"
                                        required
                                        @change="$v.garantia.$touch()"
                                        @blur="$v.garantia.$touch()"
                                    ></v-text-field>
                                </v-flex>
...
<script>
    ...
    export default {
        ...
        props:['adicional'],
        created() {
            this.ferramentas   = this.adicional.ferramentas
            this.extintor    = this.adicional.extintor

            this.assistencia = this.adicional.assistencia
            this.garantia  = this.adicional.garantia
        ...
        computed: {
            ...
            ferramentasErrors(){
                const errors = []
                if (!this.$v.ferramentas.$dirty) return errors
                !this.$v.ferramentas.required && errors.push('Conservatoria de registo predial é obrigtorio')
                return errors
            },

            extintorErrors(){
                const errors = []
                if (!this.$v.extintor.$dirty) return errors
                !this.$v.extintor.required && errors.push('Conservatoria de registo fiscal é obrigtorio')
                return errors
            },

            assistenciaErrors(){
                const errors = []
                if (!this.$v.assistencia.$dirty) return errors
                !this.$v.assistencia.required && errors.push('Nr de registro é obrigtorio')
                return errors
            },
            ...
            garantiaErrors(){
                ...
                !this.$v.garantia.required && errors.push('Garantia é obrigtorio')
                return errors
            },
